...
const Stake: React.FC = () => {
  ...
  const [approveStatus, approve] = useApprove(
    basisGold.BSGS,
    basisGold.boardroomByVersion(boardroomVersion).address,
  );

  const tokenBalance = useTokenBalance(basisGold.BSGS);
  const stakedBalance = useStakedBalanceOnBoardroom();
  const isOldBoardroomMember = boardroomVersion !== 'latest';

  const { onStake } = useStakeToBoardroom();
  const { onWithdraw } = useWithdrawFromBoardroom();
  const { onRedeem } = useRedeemOnBoardroom('Redeem BSGS for Boardroom Migration');
      ...
      <CardContent>
        <StyledCardContentInner>
          <StyledCardHeader>
            ...
            <Label text="Basis Gold Share Staked" />
          </StyledCardHeader>
          <StyledCardActions>
